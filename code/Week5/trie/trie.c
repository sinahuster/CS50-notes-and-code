...
bool check(char *word)
{
    // Create a new node, to track progress through the word
    node *cursor = root;
    ...
    for (int i = 0; i < strlen(word); i++)
    {
        ...
        // If no node exsists with this character, return false as the word doesn't exsist
        if(cursor->children[index] == NULL)
        {
            return false;
        }

        // Move the cursor to the child, character
        cursor = cursor->children[index];
    }
    return true;
}
